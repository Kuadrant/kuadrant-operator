...
test-bench: ## Run benchmarks.
	go test ./internal/controller/... -bench=. -benchmem -benchtime=200ms -run='^$$' -count=10 -timeout 10m -tags bench | tee benchmark-results.txt
...
INTEGRATION_CONFIGS := \
	...
	controlplane:local-gatewayapi-env-setup:test-controlplane-integration: \
