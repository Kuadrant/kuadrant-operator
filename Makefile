...
WASM_SHIM_VERSION ?= v0.4.0-alpha.3
$(WASM_SHIM):
	@{ \
	ASSET_ID=$$(curl -s -H "Accept: application/vnd.github.v3.raw" \
		https://api.github.com/repos/Kuadrant/wasm-shim/releases | \
		jq ". | map(select(.tag_name == \"$(WASM_SHIM_VERSION)\"))[0].assets | map(select(.name == \"kuadrant-wasm-shim-$(WASM_SHIM_VERSION)\"))[0].id" \
	) \
	&& echo "Downloading kuadrant-wasm-shim@$(WASM_SHIM_VERSION) from https://api.github.com/repos/Kuadrant/wasm-shim/releases/assets/$${ASSET_ID}" \
	&& curl -sSLo $@ -H "Accept: application/octet-stream" https://api.github.com/repos/Kuadrant/wasm-shim/releases/assets/$${ASSET_ID}; \
	}
	sha256sum $@

.PHONY: wasm-shim
wasm-shim: $(WASM_SHIM) ## Download the wasm-shim locally if necessary.
