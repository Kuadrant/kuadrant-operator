...
// +kubebuilder:validation:XValidation:rule="!(has(self.overrides) || has(self.defaults)) ? size(self.limits) > 0 : true",message="spec.limits must be defined"
// +kubebuilder:validation:XValidation:rule="has(self.overrides) ? size(self.overrides.limits) > 0 : true",message="spec.overrides.limits must be defined"
// +kubebuilder:validation:XValidation:rule="has(self.defaults) ? size(self.defaults.limits) > 0 : true",message="spec.defaults.limits must be defined"
