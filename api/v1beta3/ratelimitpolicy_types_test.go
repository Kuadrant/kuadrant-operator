...
func TestConvertRateIntoSeconds(t *testing.T) {
	testCases := []struct {
		...
		{
			name:             "seconds",
			rate:             Rate{Limit: 5, Duration: Duration("2s")},
			expectedMaxValue: 5,
			expectedSeconds:  2,
		},
		{
			name:             "minutes",
			rate:             Rate{Limit: 5, Duration: Duration("2m")},
			expectedMaxValue: 5,
			expectedSeconds:  2 * 60,
		},
		{
			name:             "hours",
			rate:             Rate{Limit: 5, Duration: Duration("2h")},
			expectedMaxValue: 5,
		...
		{
			name:             "limit  is 0",
			rate:             Rate{Limit: 0, Duration: Duration("2s")},
			expectedMaxValue: 0,
			expectedSeconds:  2,
		},
		{
			name:             "rate is 0",
			rate:             Rate{Limit: 5, Duration: Duration("0s")},
			expectedMaxValue: 5,
			expectedSeconds:  0,
		},
		{
			name:             "invalid duration 01",
			rate:             Rate{Limit: 5, Duration: Duration("unknown")},
			expectedMaxValue: 5,
			expectedSeconds:  0,
		},
		{
			name:             "invalid duration 02",
			rate:             Rate{Limit: 5, Duration: Duration("5d")},
			expectedMaxValue: 5,
			expectedSeconds:  0,
		},
	}
