...
func (r *OIDCPolicyReconciler) Reconcile(ctx context.Context, request reconcile.Request, kCtx types.KuadrantCtx) (reconcile.Result, error) {
	...
	ingressGatewayData, err := extcontroller.Resolve[ingressGatewayInfo](ctx, kCtx, oidcPolicy, "{\"protocol\": self.findGateways()[0].spec.listeners[0].protocol, \"hostname\": self.findGateways()[0].spec.listeners[0].hostname, \"name\": self.findGateways()[0].metadata.name, \"namespace\": self.findGateways()[0].metadata.namespace}", true)
	if err != nil {
