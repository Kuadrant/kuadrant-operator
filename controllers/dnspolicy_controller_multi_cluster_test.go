...
var _ = Describe("DNSPolicy Multi Cluster", func() {
	...
	BeforeEach(func() {
		...
		//clusterOneIDHash = common.ToBase36HashLen(fmt.Sprintf("%s-%s-%s", TestClusterNameOne, gateway.Name, gateway.Namespace), utils.ClusterIDLength)
		//clusterTwoIDHash = common.ToBase36HashLen(fmt.Sprintf("%s-%s-%s", TestClusterNameTwo, gateway.Name, gateway.Namespace), utils.ClusterIDLength)
