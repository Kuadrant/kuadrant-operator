...
func (r *RateLimitingWASMPluginReconciler) topologyIndexesFromGateway(ctx context.Context, gw *gatewayapiv1.Gateway) (*kuadrantgatewayapi.TopologyIndexes, error) {
	...
	err = r.Client().List(ctx, routeList, client.MatchingFields{controllers.HTTPRouteGatewayParentField: client.ObjectKeyFromObject(gw).String()})
...
func addHTTPRouteByGatewayIndexer(mgr ctrl.Manager, baseLogger logr.Logger) error {
	if err := mgr.GetFieldIndexer().IndexField(context.Background(), &gatewayapiv1.HTTPRoute{}, controllers.HTTPRouteGatewayParentField, func(rawObj client.Object) []string {
