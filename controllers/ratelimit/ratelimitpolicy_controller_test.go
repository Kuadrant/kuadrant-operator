...
var _ = Describe("RateLimitPolicy controller", func() {
	...
	assertPolicyIsAcceptedAndEnforced := func(ctx context.Context, key client.ObjectKey) func() bool {
		return func() bool {
			return test.RLPIsAccepted(ctx, k8sClient, key)() && test.RLPIsEnforced(ctx, k8sClient, key)()
		}
	}
	...
	Context("RLP targeting HTTPRoute", func() {
		It("Creates all the resources for a basic HTTPRoute and RateLimitPolicy", func(ctx SpecContext) {
			// create httproute
			httpRoute := test.BuildBasicHTTPRoute(routeName, test.GatewayName, testNamespace, []string{"*.example.com"})
			err := k8sClient.Create(ctx, httpRoute)
			Expect(err).ToNot(HaveOccurred())
			Eventually(test.RouteIsAccepted(k8sClient, client.ObjectKeyFromObject(httpRoute))).WithContext(ctx).Should(BeTrue())
			...
			gwKey := client.ObjectKey{Name: test.GatewayName, Namespace: testNamespace}
	...
	Context("RLP targeting Gateway", func() {
			...
			rlp := &kuadrantv1beta2.RateLimitPolicy{
				...
				Spec: kuadrantv1beta2.RateLimitPolicySpec{
					TargetRef: gatewayapiv1alpha2.PolicyTargetReference{
						...
						Kind:  "Gateway",
						Name:  gatewayapiv1.ObjectName(test.GatewayName),
					},
					Defaults: &kuadrantv1beta2.RateLimitPolicyCommonSpec{
						Limits: map[string]kuadrantv1beta2.Limit{
							"l1": {
		...
		It("Creates all the resources for a basic Gateway and RateLimitPolicy when missing a HTTPRoute attached to the Gateway", func(ctx SpecContext) {
			...
			Expect(test.RLPEnforcedCondition(ctx, k8sClient, rlpKey, kuadrant.PolicyReasonUnknown, "RateLimitPolicy has encountered some issues: no free routes to enforce policy"))
	...
	Context("RLP Defaults", func() {
		Describe("Route policy defaults taking precedence over Gateway policy defaults", func() {
			...
			BeforeEach(func(ctx SpecContext) {
				...
				Eventually(test.RLPIsEnforced(ctx, k8sClient, gwRLPKey)).WithContext(ctx).Should(BeFalse())
			...
			When("Free route is created", func() {
				It("Gateway policy should now be enforced", func(ctx SpecContext) {
					route2 := test.BuildBasicHTTPRoute("route2", test.GatewayName, testNamespace, []string{"*.car.com"})
					Expect(k8sClient.Create(ctx, route2)).To(Succeed())
					Eventually(test.RLPIsEnforced(ctx, k8sClient, client.ObjectKeyFromObject(gwRLP))).WithContext(ctx).Should(BeTrue())
				}, testTimeOut)
			})
		...
		It("Explicit defaults - no underlying routes to enforce policy", func(ctx SpecContext) {
			...
			Expect(test.RLPEnforcedCondition(ctx, k8sClient, gwRLPKey, kuadrant.PolicyReasonUnknown, "RateLimitPolicy has encountered some issues: no free routes to enforce policy"))
		}, testTimeOut)
	...
	Context("RLP Overrides", func() {
		...
		It("Gateway atomic override - gateway overrides exist and then route policy created", func(ctx SpecContext) {
			...
			Expect(test.RLPEnforcedCondition(ctx, k8sClient, routeRLPKey, kuadrant.PolicyReasonOverridden, fmt.Sprintf("RateLimitPolicy is overridden by [%s]", gwRLPKey)))
			...
			Eventually(test.RLPIsEnforced(ctx, k8sClient, routeRLPKey)).WithContext(ctx).Should(BeTrue())
		...
		It("Gateway atomic override - route policy exits and then gateway policy created", func(ctx SpecContext) {
			...
			Eventually(test.RLPIsEnforced(ctx, k8sClient, routeRLPKey)).WithContext(ctx).Should(BeFalse())
			Expect(test.RLPEnforcedCondition(ctx, k8sClient, routeRLPKey, kuadrant.PolicyReasonOverridden, fmt.Sprintf("RateLimitPolicy is overridden by [%s]", gwRLPKey)))
		...
		It("Gateway atomic override - gateway defaults turned into overrides later on", func(ctx SpecContext) {
			...
			gwRLPKey := client.ObjectKeyFromObject(gwRLP)
			Eventually(assertPolicyIsAcceptedAndNotEnforced(ctx, gwRLPKey)).WithContext(ctx).Should(BeTrue())
			Expect(test.RLPEnforcedCondition(ctx, k8sClient, gwRLPKey, kuadrant.PolicyReasonOverridden, fmt.Sprintf("RateLimitPolicy is overridden by [%s]", routeRLPKey)))
			...
			Eventually(test.RLPIsEnforced(ctx, k8sClient, routeRLPKey)).WithContext(ctx).Should(BeFalse())
			Expect(test.RLPEnforcedCondition(ctx, k8sClient, routeRLPKey, kuadrant.PolicyReasonOverridden, fmt.Sprintf("RateLimitPolicy is overridden by [%s]", gwRLPKey)))
			Eventually(test.RLPIsEnforced(ctx, k8sClient, gwRLPKey)).WithContext(ctx).Should(BeTrue())
	...
	Context("RLP accepted condition reasons", func() {
		...
		It("Conflict reason", func(ctx SpecContext) {
			...
			rlp := policyFactory()
			Expect(k8sClient.Create(ctx, rlp)).To(Succeed())
			Eventually(test.RLPIsAccepted(ctx, k8sClient, client.ObjectKeyFromObject(rlp))).WithContext(ctx).Should(BeTrue())
	...
	Context("When RLP switches target from one HTTPRoute to another HTTPRoute", func() {
		...
		It("direct references are updated", func(ctx SpecContext) {
			...
			// create httproute A
			httpRouteA := test.BuildBasicHTTPRoute(routeAName, test.GatewayName, testNamespace, []string{"*.a.example.com"})
			err := k8sClient.Create(ctx, httpRouteA)
			Expect(err).ToNot(HaveOccurred())
			Eventually(test.RouteIsAccepted(k8sClient, client.ObjectKeyFromObject(httpRouteA))).WithContext(ctx).Should(BeTrue())
			...
			Eventually(
				test.HTTPRouteHasDirectBackReference(k8sClient,
			...
			// create httproute B
			httpRouteB := test.BuildBasicHTTPRoute(routeBName, test.GatewayName, testNamespace, []string{"*.b.example.com"})
			err = k8sClient.Create(ctx, httpRouteB)
			Expect(err).ToNot(HaveOccurred())
			Eventually(test.RouteIsAccepted(k8sClient, client.ObjectKeyFromObject(httpRouteB))).WithContext(ctx).Should(BeTrue())
			...
			Eventually(
				test.HTTPRouteWithoutDirectBackReference(k8sClient, routeAKey, rlp.DirectReferenceAnnotationName())).WithContext(ctx).Should(BeTrue())
	...
	Context("When RLP switches target from one Gateway to another Gateway", func() {
		...
		It("direct references are updated", func(ctx SpecContext) {
			...
			// create Gw A
			gatewayA := test.BuildBasicGateway(gwAName, testNamespace)
			err := k8sClient.Create(ctx, gatewayA)
			Expect(err).ToNot(HaveOccurred())
			Eventually(test.GatewayIsReady(k8sClient, gatewayA)).WithContext(ctx).Should(BeTrue())
			...
			Eventually(
				test.GatewayHasDirectBackReference(k8sClient,
			...
			// create Gw B
			gatewayB := test.BuildBasicGateway(gwBName, testNamespace)
			err = k8sClient.Create(ctx, gatewayB)
			Expect(err).ToNot(HaveOccurred())
			Eventually(test.GatewayIsReady(k8sClient, gatewayB)).WithContext(ctx).Should(BeTrue())
			...
			Eventually(
				test.GatewayWithoutDirectBackReference(k8sClient, gwAKey, rlp.DirectReferenceAnnotationName())).WithContext(ctx).Should(BeTrue())
	...
	Context("When RLP switches target from one HTTPRoute to another taken HTTPRoute", func() {
		...
		It("direct references are updated", func(ctx SpecContext) {
			...
			Eventually(test.RLPIsNotAccepted(ctx, k8sClient, rlpAKey)).WithContext(ctx).Should(BeTrue())

			// Check HTTPRoute A direct back reference is gone
			Eventually(
				test.HTTPRouteWithoutDirectBackReference(k8sClient, routeAKey, rlpA.DirectReferenceAnnotationName())).WithContext(ctx).Should(BeTrue())
	...
	Context("When target is deleted", func() {
		...
		It("policy status reports error", func(ctx SpecContext) {
			...
			Expect(err).ToNot(HaveOccurred())
			Eventually(test.ObjectDoesNotExist(k8sClient, httpRoute)).WithContext(ctx).Should(BeTrue())

			// Check RLP status is available
			Eventually(test.RLPIsNotAccepted(ctx, k8sClient, rlpKey)).WithContext(ctx).Should(BeTrue())
		}, testTimeOut)
	})
	...
	Context("When RLP targets already taken HTTPRoute and the route is being released", func() {
		...
		It("direct references are updated and RLP status is ready", func(ctx SpecContext) {
			...
			rlpBKey := client.ObjectKeyFromObject(rlpB)
			Eventually(test.RLPIsNotAccepted(ctx, k8sClient, rlpBKey)).WithContext(ctx).Should(BeTrue())
