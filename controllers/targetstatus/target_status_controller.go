...
// Reconciler reconciles a the status stanzas of objects targeted by Kuadrant policies
...
func (r *Reconciler) reconcileResources(ctx context.Context, gw *gatewayapiv1.Gateway) error {
...
func (r *Reconciler) reconcileResourcesForPolicyKind(parentCtx context.Context, gw *gatewayapiv1.Gateway, policy kuadrantgatewayapi.Policy, listPolicyKind client.ObjectList) error {
	...
	topology, err := controllers.BuildTopology(ctx, r.Client(), gw, policyKind, listPolicyKind)
	if err != nil {
		return err
	}
...
func (r *Reconciler) updateInheritedGatewayCondition(ctx context.Context, routes []*gatewayapiv1.HTTPRoute, condition metav1.Condition, gatewayKey client.ObjectKey, update updateRouteConditionFunc) error {
...
func (r *Reconciler) addRouteCondition(ctx context.Context, route *gatewayapiv1.HTTPRoute, condition metav1.Condition, gatewayKey client.ObjectKey, controllerName gatewayapiv1.GatewayController) error {
	logger, _ := logr.FromContext(ctx)
...
func (r *Reconciler) removeRouteCondition(ctx context.Context, route *gatewayapiv1.HTTPRoute, condition metav1.Condition, gatewayKey client.ObjectKey, controllerName gatewayapiv1.GatewayController) error {
	logger, _ := logr.FromContext(ctx)
	logger = logger.WithValues("route", client.ObjectKeyFromObject(route), "condition", condition.Type, "status", condition.Status)
...
func buildPolicyAffectedCondition(policy kuadrantgatewayapi.Policy) metav1.Condition {
	...
	if c := meta.FindStatusCondition(policy.GetStatus().GetConditions(), string(gatewayapiv1alpha2.PolicyConditionAccepted)); c == nil || c.Status != metav1.ConditionTrue { // should we aim for 'Enforced' instead?
		...
		condition.Reason = string(kuadrant.PolicyReasonUnknown)
		if c != nil {
