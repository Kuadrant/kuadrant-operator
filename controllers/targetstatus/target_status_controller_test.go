...
var _ = Describe("Target status reconciler", func() {
	...
	routeAffected := func(ctx context.Context, routeName, conditionType string, policyKey client.ObjectKey) bool {
		...
		routeParentStatus, found := utils.Find(route.Status.RouteStatus.Parents, findRouteParentStatusFunc(route, client.ObjectKey{Name: test.GatewayName, Namespace: testNamespace}, kuadrant.ControllerName))
		if !found {
			return false
		}
	...
	Context("AuthPolicy", func() {
		...
		policyAcceptedAndTargetsAffected := func(ctx context.Context, policy *v1beta2.AuthPolicy, routeNames ...string) func() bool {
			return func() bool {
				if !test.IsAuthPolicyAccepted(ctx, k8sClient, policy)() {
					return false
				}
		...
		It("Adds truthy PolicyAffected status condition if there is at least one policy accepted", func(ctx SpecContext) {
			...
			Eventually(func() bool {
				return policyAcceptedAndTargetsAffected(ctx, routePolicy1)() &&
					!test.IsAuthPolicyAccepted(ctx, k8sClient, routePolicy2)() && !routeAffected(ctx, test.HTTPRouteName, policyAffectedCondition, client.ObjectKeyFromObject(routePolicy2))
			}).WithContext(ctx).Should(BeTrue())
		}, testTimeOut)

		It("Adds falsey PolicyAffected status condition if no policy is accepted", func(ctx SpecContext) {
			routePolicy1 := policyFactory(func(p *v1beta2.AuthPolicy) { // create a policy with an invalid route selectorso the policy is not accepted
				p.Name = "route-auth-1"
			...
			Eventually(func() bool {
				return !test.IsAuthPolicyAccepted(ctx, k8sClient, routePolicy1)() && routeNotAffected(ctx, test.HTTPRouteName, policyAffectedCondition, client.ObjectKeyFromObject(routePolicy1)) &&
					!test.IsAuthPolicyAccepted(ctx, k8sClient, routePolicy2)() && !routeAffected(ctx, test.HTTPRouteName, policyAffectedCondition, client.ObjectKeyFromObject(routePolicy2))
			}).WithContext(ctx).Should(BeTrue())
		}, testTimeOut)
		...
		It("removes PolicyAffected status condition from the targeted route when the policy is deleted", func(ctx SpecContext) {
			...
			Eventually(func() bool { // route is not affected by the policy
				route := &gatewayapiv1.HTTPRoute{}
				err := k8sClient.Get(ctx, client.ObjectKey{Name: test.HTTPRouteName, Namespace: testNamespace}, route)
				if err != nil {
					return false
				}
				routeParentStatus, found := utils.Find(route.Status.RouteStatus.Parents, findRouteParentStatusFunc(route, client.ObjectKey{Name: test.GatewayName, Namespace: testNamespace}, kuadrant.ControllerName))
		...
		It("adds PolicyAffected status condition to the targeted gateway and routes", func(ctx SpecContext) {
			...
			Expect(k8sClient.Create(ctx, policy)).To(Succeed())
			Eventually(policyAcceptedAndTargetsAffected(ctx, policy, test.HTTPRouteName)).WithContext(ctx).Should(BeTrue())
		}, testTimeOut)
		...
		It("removes PolicyAffected status condition from the targeted gateway and routes when the policy is deleted", func(ctx SpecContext) {
			...
			Eventually(func() bool { // gateway and route not affected by the policy
				gateway := &gatewayapiv1.Gateway{}
				err := k8sClient.Get(ctx, client.ObjectKey{Name: test.GatewayName, Namespace: testNamespace}, gateway)
		...
		It("adds PolicyAffected status condition to the targeted gateway and non-targeted routes", func(ctx SpecContext) {
			...
			Eventually(func() bool {
				return test.RouteIsAccepted(k8sClient, client.ObjectKeyFromObject(otherRoute))() &&
			...
			Eventually(policyAcceptedAndTargetsAffected(ctx, gatewayPolicy, otherRouteName, test.HTTPRouteName)).WithContext(ctx).Should(BeTrue())
		}, testTimeOut)
	})
	...
	Context("RateLimitPolicy", func() {
			...
			policy := &v1beta2.RateLimitPolicy{
				...
				Spec: v1beta2.RateLimitPolicySpec{
					TargetRef: gatewayapiv1alpha2.PolicyTargetReference{
						Group: gatewayapiv1.GroupName,
						Kind:  "HTTPRoute",
						Name:  gatewayapiv1.ObjectName(test.HTTPRouteName),
					},
		...
		policyAcceptedAndTargetsAffected := func(ctx context.Context, policy *v1beta2.RateLimitPolicy, routeNames ...string) func() bool {
			return func() bool {
				...
				if !test.RLPIsAccepted(ctx, k8sClient, policyKey)() {
					return false
				}
	...
	Context("DNSPolicy", func() {
		...
		policyFactory := func(mutateFns ...func(policy *v1alpha1.DNSPolicy)) *v1alpha1.DNSPolicy {
			policy := v1alpha1.NewDNSPolicy("test-dns-policy", testNamespace).WithTargetGateway(test.GatewayName).WithRoutingStrategy(v1alpha1.SimpleRoutingStrategy)
			for _, mutateFn := range mutateFns {
				mutateFn(policy)
			}
			return policy
		...
		It("removes PolicyAffected status condition from the targeted gateway when the policy is deleted", func(ctx SpecContext) {
			...
			Eventually(func() bool {
				...
				condition := meta.FindStatusCondition(gateway.Status.Conditions, test.GatewayName)
	...
	Context("TLSPolicy", func() {
		...
		policyFactory := func(mutateFns ...func(policy *v1alpha1.TLSPolicy)) *v1alpha1.TLSPolicy {
			policy := v1alpha1.NewTLSPolicy("test-tls-policy", testNamespace).WithTargetGateway(test.GatewayName).WithIssuerRef(*issuerRef)
			for _, mutateFn := range mutateFns {
				mutateFn(policy)
			}
			return policy
		...
		BeforeEach(func(ctx SpecContext) {
			issuer, issuerRef = test.BuildSelfSignedIssuer("testissuer", testNamespace)
