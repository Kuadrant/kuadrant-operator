...
#### Current Approach: Same-Container Deployment

To integrate your extension into the operator deployment (like PlanPolicy and OIDCPolicy):

1. **Update Makefile**: Add your extension to `WITH_EXTENSIONS`
2. **Update Dockerfile**: Build and include your extension binary
3. **Create RBAC manifests**: Define required permissions in `config/rbac/`
4. **Generate manifests**: Run `make bundle` and `make helm-build`

This approach runs the extension as a separate process in the same container as the operator, communicating over Unix domain sockets.
