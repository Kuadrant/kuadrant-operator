...
- **`upstream_cluster`** (`%UPSTREAM_CLUSTER_RAW%`): The upstream service that handled the request (Istio 1.23+; use `%UPSTREAM_CLUSTER%` on older versions)
...
| `%UPSTREAM_CLUSTER_RAW%` | Upstream cluster name (Istio 1.23+; use `%UPSTREAM_CLUSTER%` on older versions) |
