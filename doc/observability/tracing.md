...
- `defaultEndpoint`: The URL of the tracing collector backend. Format: `<scheme>://<hostname>:<port>`
  - `rpc://` - OTLP over gRPC (port 4317)
...
spec:
  tracing:
    endpoint: rpc://jaeger-collector.jaeger.svc.cluster.local:4317
    insecure: true
---
...
Traces the operator's reconciliation loops and internal operations:
- Shows policy lifecycle events, topology building, resource creation
- Helps debug operator behavior and performance
- Service name in traces: `kuadrant-operator`

### Data Plane Tracing

Traces actual user requests through the gateway and policy enforcement components:
...
- Service names in traces: `istio-proxy`, `authorino`, `limitador`
...
Data plane tracing captures actual user requests as they flow through the gateway and policy enforcement components. When enabled via the Kuadrant CR configuration (see above), traces are automatically collected from:

- **Istio/Envoy Gateway**: Gateway-level request routing and processing
- **Authorino**: Authentication and authorization decisions
- **Limitador**: Rate limiting checks
- **WASM filters**: Gateway-level policy enforcement

### Viewing Data Plane Traces
...
Here is an example trace in the Grafana UI showing the total request time from the gateway (Istio), the time to check the current rate limit count (and update it) in Limitador and the time to check auth in Authorino:

![Trace in Grafana UI](grafana_trace.png)

### Correlating Data Plane Traces with Logs
...
Using a combination of tracing and logs, you can visualize and troubleshoot request timing issues and drill down to specific services.
...
**Policy Reconciliation Workflow Spans:**

The `workflow.data_plane_policies` workflow handles reconciliation of policies that affect the data plane (AuthPolicy, RateLimitPolicy, TokenRateLimitPolicy):
