...
*Note*: If you want to deploy Kuadrant with a custom gateway controller name you need to update the CSV to set the `ISTIO_GATEWAY_CONTROLLER_NAMES`
or `ENVOYGATEWAY_GATEWAY_CONTROLLER_NAMES` environment variable in the kuadrant controller manager.

```
kubectl patch csv <CSV_NAME> -n <NAMESPACE> --type=json -p='[{"op":"add","path":"/spec/install/spec/deployments/0/spec/template/spec/containers/0/env/-","value":{"name":"ISTIO_GATEWAY_CONTROLLER_NAMES","value":"istio.io/gateway-controller,openshift.io/gateway-controller/v1"}}]'
```
