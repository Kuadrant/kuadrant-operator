...
| `helm lint charts/kuadrant-operator` | Lint | Validates chart structure and syntax |
| `helm template charts/kuadrant-operator` | Test Rendering | Tests template rendering without installation |
| `make helm-package` | Package | Creates .tgz file for local testing |
| `helm dependency build charts/kuadrant-operator` | Dependencies | Downloads chart dependencies |

### Development Workflow Steps

1. **Edit Chart Files**: Modify templates, values, or Chart.yaml in `charts/kuadrant-operator/`
2. **Build Charts**: Run `make helm-build` to generate updated templates  
3. **Test Locally**: Use `helm template` and `helm lint` to validate changes
4. **Verify Changes**: Run `make verify-helm-charts` to ensure no regressions
5. **Submit PR**: Create pull request with chart changes for review
6. **CI Validation**: Automated checks validate chart quality and compatibility

### Local Testing

```bash
# Build the helm chart from current code
make helm-build

# Lint the chart for issues
helm lint charts/kuadrant-operator

# Test template rendering
helm template kuadrant-operator charts/kuadrant-operator
...
**Trigger**: Developer manually runs the [Automated Release Workflow](../../.github/workflows/automated-release.yaml)

**Steps**:
1. **Update Versions**: The workflow updates `release.yaml` with all component versions:
...
2. **Run Prepare Release**: Executes `make prepare-release` which:
...
**Overview**: Once the release PR is ready, developers can choose between two approaches for creating the actual release:

1. **Automated Process** (Recommended): The release is created automatically when the PR is merged
2. **Manual Process**: Follow the detailed steps in [RELEASE.md](../../RELEASE.md)
...
**Steps**:
1. **Configure GPG Key**: Sets up GPG key for chart signing
2. **Build Dependencies**:
   ```bash
   make helm-add-kuadrant-repo
   make helm-dependency-build
   ```
3. **Package & Sign Chart**:
   ```bash
...
5. **Sync to Helm Charts Repo**: Executes `helm-sync-package-created`
