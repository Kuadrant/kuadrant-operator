...
- **`Reservation`** (default): On request arrival the gateway *reserves* an estimated token amount from Limitador and, once the upstream responds, *commits* the actual usage (releasing the difference). This closes the race window that exists between checking and reporting under concurrency. See [RFC 0021](https://github.com/Kuadrant/architecture/blob/main/rfcs/0021-token-rate-limit-reservations.md). Reservation behavior per limit is tuned with [`spec.limits.<name>.reservation`](tokenratelimitpolicy.md#reservation) on the `TokenRateLimitPolicy`.
