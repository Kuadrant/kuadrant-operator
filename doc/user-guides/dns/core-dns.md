...
For detailed zone configuration options, NS records, and multi-cluster setup, see [CoreDNS Configuration Reference - Zone Configuration](https://github.com/Kuadrant/dns-operator/blob/main/docs/coredns/configuration.md#zone-configuration).
...
For testing geographic routing with the demo database, see the subnet-to-location mapping table and testing instructions in the [dns-operator README - GEO Testing](https://github.com/Kuadrant/dns-operator/blob/main/docs/coredns/README.md#geo).
...
The CoreDNS configuration (Corefile) defines which plugins execute for each zone. For plugin configuration details, execution order, and complete examples, see [CoreDNS Configuration Reference - Corefile Configuration](https://github.com/Kuadrant/dns-operator/blob/main/docs/coredns/configuration.md#corefile-configuration).

#### Advanced: Customizing SOA RNAME (Optional)

To customize the SOA RNAME email address in DNS records, see [CoreDNS Configuration Reference - SOA RNAME Configuration](https://github.com/Kuadrant/dns-operator/blob/main/docs/coredns/configuration.md#soa-rname-configuration).
...
In order for each primary cluster to read DNSRecord resources from all other clusters (primary or secondary) a cluster interconnection secret containing kubeconfig data for each of the other clusters must be added.
The `kubectl kuadrant-dns` CLI provides a command to help create a valid cluster secret from the expected service account on the target cluster.

Refer to the [CLI documentation](https://github.com/Kuadrant/dns-operator/blob/main/docs/cli.md) for more information on how to install the CLI.

Assuming the `kubectl kuadrant-dns` CLI is installed (or the `kubectl-kuadrant_dns` binary is in the system path), you can run the following to connect all clusters to each primary:
...
Check the cluster interconnection secrets exist on the primary clusters:
```shell
...
Check the cluster interconnection secret content is valid kubeconfig data:
```shell
...
> **Note**: The authoritative DNSRecord name is deterministically generated from the root host. First list all DNSRecords to find the authoritative record name in your environment.

```shell
# List all DNSRecords to find the authoritative record
kubectl get dnsrecord -n dnstest --context ${CTX_PRIMARY1}
kubectl get dnsrecord -n dnstest --context ${CTX_PRIMARY2}

# Then inspect the authoritative record (replace <authoritative-record-name> with the name from above)
kubectl get dnsrecord/<authoritative-record-name> -n dnstest -o json --context ${CTX_PRIMARY1} | jq -r '.spec.endpoints[] | "dnsName: \(.dnsName), recordType: \(.recordType), targets: \(.targets)"'
kubectl get dnsrecord/<authoritative-record-name> -n dnstest -o json --context ${CTX_PRIMARY2} | jq -r '.spec.endpoints[] | "dnsName: \(.dnsName), recordType: \(.recordType), targets: \(.targets)"'
```
