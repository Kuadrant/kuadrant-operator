...
spec:
  ...
  limits:
    free:
      rates:
        - limit: 20000      # 20k tokens per day for free users
          window: 24h
      when:
        - predicate: >
    ...
    gold:
      rates:
        - limit: 200000     # 200k tokens per day for gold users
          window: 24h
      when:
        - predicate: >
...
- **Token extraction**: Automatically reads `usage.total_tokens` from JSON responses
...
- Set up monitoring to track token usage patterns
...
4. Review the `AuthPolicy` and `TokenRateLimitPolicy` status
