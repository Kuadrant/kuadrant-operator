...
Delete the example RateLimitPolicies (remove only the policies you deployed):

```bash
kubectl delete ratelimitpolicy grpcbin-rlp 2>/dev/null || true
kubectl delete ratelimitpolicy grpcbin-rlp-no-reflection 2>/dev/null || true
kubectl delete ratelimitpolicy grpcbin-method-rlp 2>/dev/null || true
```

Or delete using the manifest files:

```bash
kubectl delete -f examples/grpc-backend/ratelimitpolicy/ratelimitpolicy.yaml 2>/dev/null || true
kubectl delete -f examples/grpc-backend/ratelimitpolicy/ratelimitpolicy-no-reflection.yaml 2>/dev/null || true
kubectl delete -f examples/grpc-backend/ratelimitpolicy/ratelimitpolicy-method.yaml 2>/dev/null || true
```
