...
func (r *HelmLimitadorOperatorReconciler) Reconcile(ctx context.Context, _ []controller.ResourceEvent, topology *machinery.Topology, _ error, state *sync.Map) error {
	...
	objects, err := renderer.Render("limitador-operator", operatorNamespace, nil)
	if err != nil {
	...
	if err := applyResources(ctx, r.Client, objects, operatorNamespace, logger); err != nil {
		span.RecordError(err)
		span.SetStatus(codes.Error, "failed to apply limitador-operator resources")
		return err
	}

	span.SetStatus(codes.Ok, "")
	logger.Info("limitador-operator reconciled successfully")
