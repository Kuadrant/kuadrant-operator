...
func (r *IstioExtensionReconciler) Reconcile(ctx context.Context, _ []controller.ResourceEvent, topology *machinery.Topology, _ error, state *sync.Map) error {
	...
	for _, gateway := range gateways {
		...
		desiredWasmPlugin := buildIstioWasmPluginForGateway(gateway, wasmConfig, resolvedImageURL, useImagePullSecret)
...
func buildIstioWasmPluginForGateway(gateway *machinery.Gateway, wasmConfig wasm.Config, imageURL string, useImagePullSecret bool) *istioclientgoextensionv1alpha1.WasmPlugin {
