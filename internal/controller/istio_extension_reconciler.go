...
func (r *IstioExtensionReconciler) buildWasmConfigs(ctx context.Context, topology *machinery.Topology, state *sync.Map) (map[string]wasm.Config, error) {
	...
	defer logger.Info("build Wasm configuration", "status", "completed")

	var authClusterName string
	authorino := GetAuthorinoFromTopology(topology)
	if authorino != nil {
		authorinoServiceSpec := ServiceSpecFromAuthorino(authorino)
		authClusterName = authorinoServiceSpec.ToClusterName()
	}

	var ratelimitClusterName string
	limitador := GetLimitadorFromTopology(topology)
	if limitador != nil {
		limitadorServiceSpec := ServiceSpecFromLimitador(limitador)
		ratelimitClusterName = limitadorServiceSpec.ToClusterName()
	}
	...
	wasmConfigs := lo.MapValues(wasmActionSets.Sorted(), func(configs kuadrantgatewayapi.SortableHTTPRouteMatchConfigs, _ string) wasm.Config {
		...
		}), &logger, authClusterName, ratelimitClusterName)
	})
