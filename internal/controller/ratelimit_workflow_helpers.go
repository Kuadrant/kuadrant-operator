...
// ToActionScope converts a LimitNamespace to an ActionScope.
func (ln LimitNamespace) ToActionScope() ActionScope {
	return ActionScope(ln)
}

// wasmActionSpecFromLimit builds a wasm rate-limit action spec for a given limit.
...
func wasmActionSpecFromLimit(limit *kuadrantv1.Limit, limitIdentifier string, scope ActionScope, sourcePolicyLocator string, topLevelPredicates kuadrantv1.WhenPredicates) wasm.ActionSpec {
	return wasm.ActionSpec{
		ServiceName: wasm.RateLimitServiceName,
		Scope:       string(scope),
		Sources:     []string{sourcePolicyLocator}, // Single policy for individual rules
...
// resolveReservationAmount wraps user-provided expressions in uint() because ReserveRequest.amount
// is a uint64 and the data plane rejects int values (e.g. the literal "0") for that field.
func resolveReservationAmount(reservation *kuadrantv1alpha1.Reservation) string {
	if reservation != nil && reservation.Amount != nil && *reservation.Amount != "" {
		return fmt.Sprintf("uint(%s)", *reservation.Amount)
	}
...
func wasmActionSpecsFromTokenLimit(tokenLimit *kuadrantv1alpha1.TokenLimit, limitIdentifier string, scope ActionScope, sourcePolicyLocator string, topLevelPredicates kuadrantv1.WhenPredicates, mode kuadrantv1beta1.TokenRateLimitingMode, backendTimeout *gatewayapiv1.Duration) []wasm.ActionSpec {
	...
	reservationStorePath := wasm.ReservationStorePath
	if limitIdentifier != "" {
		reservationStorePath = fmt.Sprintf("kuadrant.internal.reservation.%s", strings.ReplaceAll(limitIdentifier, ".", "_"))
	}

	requestPhaseData := make([]wasm.DataType, 0, len(commonData))
	requestPhaseData = append(requestPhaseData, commonData...)
	...
	requestSpec := wasm.ActionSpec{
		...
		Scope:                string(scope),
		Sources:              []string{sourcePolicyLocator},
		ReservationAmount:    amountVal,
		ReservationTTL:       ttlVal,
		ReservationStorePath: reservationStorePath,
		ConditionalData: []wasm.ConditionalData{
			{
				Predicates: predicates,
				Data:       requestPhaseData,
	...
	responsePhaseData := make([]wasm.DataType, 0, len(commonData))
	responsePhaseData = append(responsePhaseData, commonData...)
...
func buildWasmActionSpecsForRateLimit(effectivePolicy EffectiveRateLimitPolicy, policyPredicate func(machinery.Policy) bool) []wasm.ActionSpec {
	return buildWasmActionSpecsForAnyRateLimit(
		...
		func(spec interface{}, limitIdentifier string, scope ActionScope, sourcePolicyLocator string, predicates kuadrantv1.WhenPredicates) wasm.ActionSpec {
			limit := spec.(*kuadrantv1.Limit)
			return wasmActionSpecFromLimit(limit, limitIdentifier, scope, sourcePolicyLocator, predicates)
		},
	)
}
...
func buildWasmActionSpecsForTokenRateLimit(effectivePolicy EffectiveTokenRateLimitPolicy, policyPredicate func(machinery.Policy) bool, mode kuadrantv1beta1.TokenRateLimitingMode) []wasm.ActionSpec {
	...
	for _, r := range limitRules {
		...
		if parsed.HTTPRouteRule != nil && parsed.HTTPRouteRule.Timeouts != nil {
			backendTimeout = parsed.HTTPRouteRule.Timeouts.BackendRequest
		}
...
func buildWasmActionSpecsForAnyRateLimit(
