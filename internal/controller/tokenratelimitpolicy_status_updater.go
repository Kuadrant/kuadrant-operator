...
func (r *TokenRateLimitPolicyStatusUpdater) enforcedCondition(policy *kuadrantv1alpha1.TokenRateLimitPolicy, topology *machinery.Topology, state *sync.Map) *metav1.Condition {
	...
	for _, g := range affectedGateways {
		...
		case defaultIstioGatewayControllerName:
			// EnvoyFilter
			istioTokenRateLimitClustersModifiedGateways, _ := state.Load(StateIstioTokenRateLimitClustersModified)
			componentsToSync = append(componentsToSync, gatewayComponentsToSync(g.gateway, kuadrantistio.EnvoyFilterGroupKind, istioTokenRateLimitClustersModifiedGateways, topology, func(_ machinery.Object) bool {
		...
		case defaultEnvoyGatewayGatewayControllerName:
			...
			// EnvoyPatchPolicy
			envoyGatewayTokenRateLimitClustersModifiedGateways, _ := state.Load(StateEnvoyGatewayTokenRateLimitClustersModified)
			componentsToSync = append(componentsToSync, gatewayComponentsToSync(g.gateway, kuadrantenvoygateway.EnvoyPatchPolicyGroupKind, envoyGatewayTokenRateLimitClustersModifiedGateways, topology, func(obj machinery.Object) bool {
