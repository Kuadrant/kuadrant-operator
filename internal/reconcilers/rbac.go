...
func RoleRulesMutator(desired, existing *rbacv1.Role) bool {
	if !rulesEqual(existing.Rules, desired.Rules) {
		existing.Rules = desired.Rules
		return true
	}
...
type RoleMutateFn func(desired, existing *rbacv1.Role) bool

func RoleMutator(opts ...RoleMutateFn) MutateFn {
	return func(existingObj, desiredObj client.Object) (bool, error) {
...
func RoleBindingSubjectsMutator(desired, existing *rbacv1.RoleBinding) bool {
	update := false
...
type RoleBindingMutateFn func(desired, existing *rbacv1.RoleBinding) bool

func RoleBindingMutator(opts ...RoleBindingMutateFn) MutateFn {
	return func(existingObj, desiredObj client.Object) (bool, error) {
		existing, ok := existingObj.(*rbacv1.RoleBinding)
		if !ok {
			return false, fmt.Errorf("%T is not a *rbacv1.RoleBinding", existingObj)
		}
		desired, ok := desiredObj.(*rbacv1.RoleBinding)
		if !ok {
			return false, fmt.Errorf("%T is not a *rbacv1.RoleBinding", desiredObj)
		}
