...
# Run in series as limit of 1 Kuadrant CR (procs=1)
	OPERATOR_NAMESPACE=kuadrant-system CHARTS_PATH=$(PROJECT_PATH)/component-charts $(GINKGO) \
...
#	Check `ginkgo help run` for command line options. For example to filtering tests.
	GATEWAYAPI_PROVIDER=istio OPERATOR_NAMESPACE=kuadrant-system CHARTS_PATH=$(PROJECT_PATH)/component-charts $(GINKGO) \
		--coverpkg $(INTEGRATION_COVER_PKGS) \
...
#	Check `ginkgo help run` for command line options. For example to filtering tests.
	GATEWAYAPI_PROVIDER=envoygateway OPERATOR_NAMESPACE=kuadrant-system CHARTS_PATH=$(PROJECT_PATH)/component-charts $(GINKGO) \
		--coverpkg $(INTEGRATION_COVER_PKGS) \
...
#	Check `ginkgo help run` for command line options. For example to filtering tests.
	GATEWAYAPI_PROVIDER=$(GATEWAYAPI_PROVIDER) OPERATOR_NAMESPACE=kuadrant-system CHARTS_PATH=$(PROJECT_PATH)/component-charts $(GINKGO) \
		--coverpkg $(INTEGRATION_COVER_PKGS) \
