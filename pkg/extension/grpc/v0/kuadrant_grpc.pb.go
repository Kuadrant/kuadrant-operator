...
type ExtensionServiceClient interface {
	...
	Subscribe(ctx context.Context, in *empty.Empty, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Event], error)
...
func (c *extensionServiceClient) Subscribe(ctx context.Context, in *empty.Empty, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Event], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &ExtensionService_ServiceDesc.Streams[0], ExtensionService_Subscribe_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[empty.Empty, Event]{ClientStream: stream}
...
type ExtensionServiceServer interface {
	...
	// Subscribe to a set of Events
	Subscribe(*empty.Empty, grpc.ServerStreamingServer[Event]) error
	// Resolve the expression for context and subscribe (or not)
...
func (UnimplementedExtensionServiceServer) Subscribe(*empty.Empty, grpc.ServerStreamingServer[Event]) error {
...
func _ExtensionService_Subscribe_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(ExtensionServiceServer).Subscribe(m, &grpc.GenericServerStream[empty.Empty, Event]{ServerStream: stream})
}
