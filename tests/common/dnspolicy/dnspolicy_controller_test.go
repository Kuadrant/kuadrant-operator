...
var _ = Describe("DNSPolicy controller", func() {
	...
	It("should ensure the delegate field and providerRef are mutually exclusing", func(ctx SpecContext) {
		...
		// make sure we can create delegate=false with provider ref.
		dnsPolicy.WithDelegation(false)
		Expect(k8sClient.Create(ctx, dnsPolicy)).To(Succeed())
	}, testTimeOut)

	It("prevents invalid updates of delegate field and allows valid", func(ctx SpecContext) {
		...
		nilPolicy, err := runtime.DefaultUnstructuredConverter.ToUnstructured(dnsPolicy.DeepCopyObject())
		Expect(err).ToNot(HaveOccurred())
		// remove delegate (it will be there for go struct only if set to true)
		unstructured.RemoveNestedField(nilPolicy, "spec", "delegate")
		// remove provider ref - mutually exclusive with delegate = true
		unstructured.RemoveNestedField(nilPolicy, "spec", "providerRefs")

		falsePolicy := runtime.DeepCopyJSON(nilPolicy)
		// Set delegate to false. This can only be done while manually creating CR. Go will create nil instead
		Expect(unstructured.SetNestedField(falsePolicy, false, "spec", "delegate")).To(Succeed())

		truePolicy := runtime.DeepCopyJSON(nilPolicy)
		Expect(unstructured.SetNestedField(truePolicy, true, "spec", "delegate")).To(Succeed())
		...
		}, kuadrantv1.GroupVersion.Version)
		Expect(err).ToNot(HaveOccurred())
		Expect(mapping).ToNot(BeNil())

		fieldManager := "kuadrant"

		// create nil policy
		_, err = dynClient.
			Resource(mapping.Resource).
			Namespace(dnsPolicy.GetNamespace()).
			Apply(ctx, dnsPolicy.GetName(), &unstructured.Unstructured{Object: nilPolicy}, metav1.ApplyOptions{FieldManager: fieldManager})
		Expect(err).ToNot(HaveOccurred())
		...
		_, err = dynClient.
			Resource(mapping.Resource).
			Namespace(dnsPolicy.GetNamespace()).
			Apply(ctx, dnsPolicy.GetName(), &unstructured.Unstructured{Object: truePolicy}, metav1.ApplyOptions{FieldManager: fieldManager})
		...
		_, err = dynClient.
			Resource(mapping.Resource).
			Namespace(dnsPolicy.GetNamespace()).
			Apply(ctx, dnsPolicy.GetName(), &unstructured.Unstructured{Object: falsePolicy}, metav1.ApplyOptions{FieldManager: fieldManager})
		Expect(err).ToNot(HaveOccurred())
		...
		Eventually(func(g Gomega) {
			err = dynClient.
				Resource(mapping.Resource).
				Namespace(dnsPolicy.GetNamespace()).
				Delete(ctx, dnsPolicy.GetName(), metav1.DeleteOptions{})
			g.Expect(client.IgnoreNotFound(err)).ToNot(HaveOccurred())

			recordList, err := dynClient.
				Resource(mapping.Resource).
				Namespace(dnsPolicy.GetNamespace()).
				List(ctx, metav1.ListOptions{})
			g.Expect(recordList.Items).To(BeEmpty())
			g.Expect(err).ToNot(HaveOccurred())
		}, tests.TimeoutShort, time.Second).Should(Succeed())
	...
	Context("valid target and valid gateway status", func() {
		...
		It("should re-create dns record when loadbalanced section added/removed", func(ctx SpecContext) {
			...
			currentRecRootEndpoint := endpointMatcher(tests.HostOne(domain), "CNAME", "", builderDefaults.DefaultLoadBalancedTTL, "klb.test."+domain)
			currentWildcardRecRootEndpoint := endpointMatcher(tests.HostWildcard(domain), "CNAME", "", builderDefaults.DefaultLoadBalancedTTL, "klb."+domain)
			...
			newRecRootEndpoint := endpointMatcher(tests.HostOne(domain), "A", "", builderDefaults.DefaultTTL, tests.IPAddressOne, tests.IPAddressTwo)
			newWildcardRecRootEndpoint := endpointMatcher(tests.HostWildcard(domain), "A", "", builderDefaults.DefaultTTL, tests.IPAddressOne, tests.IPAddressTwo)
			...
			It("should update dns records when policy is updated", func(ctx SpecContext) {
				endpointMatcher := func(geo string) types.GomegaMatcher {
					return PointTo(MatchFields(IgnoreExtras, Fields{
						...
						"RecordTTL":        Equal(externaldns.TTL(builderDefaults.DefaultLoadBalancedTTL)),
			...
			It("should update dns records when gateway is updated", func(ctx SpecContext) {
				...
				endpointMatcher := func(targets ...string) types.GomegaMatcher {
					return PointTo(MatchFields(IgnoreExtras, Fields{
						...
						"RecordType":    Equal("A"),
						"SetIdentifier": Equal(""),
						"RecordTTL":     Equal(externaldns.TTL(builderDefaults.DefaultTTL)),
					}))
				}
		...
		Context("section name", func() {
			It("should handle policy with section name", func(ctx SpecContext) {
				...
				newRecRootEndpoint = endpointMatcher(tests.HostOne(domain), "CNAME", "", builderDefaults.DefaultLoadBalancedTTL, "klb.test."+domain)
				...
				//listener 1 & 2 - Default gateway policy has a loadbalancing section so will create loadbalanced records with CNAME Records for the rootHost
				newRecRootEndpoint = endpointMatcher(tests.HostOne(domain), "CNAME", "", builderDefaults.DefaultLoadBalancedTTL, "klb.test."+domain)
				newWildcardRecRootEndpoint = endpointMatcher(tests.HostWildcard(domain), "CNAME", "", builderDefaults.DefaultLoadBalancedTTL, "klb."+domain)
