...
var _ = Describe("MCP Gateway AuthPolicy integration", Ordered, Label("mcp-gateway", "long-running"), func() {
	...
	BeforeAll(func(ctx SpecContext) {
		...
		for _, route := range []*gatewayapiv1.HTTPRoute{
			...
			Eventually(tests.RouteIsAccepted(ctx, testClient(), client.ObjectKeyFromObject(route))).
				WithContext(ctx).WithTimeout(mcpAuthReadyTimeout).Should(BeTrue())
		}

		By("creating MCPServerRegistrations for the authenticated test servers")
		registrations := []*unstructured.Unstructured{
	...
	It("returns 401 for unauthenticated requests", func(ctx SpecContext) {
		status, _, headers, err := mcpAuthRawPost(ctx, mcpAuthGatewayURL, "", mcpAuthInitializeBody(), nil)
		Expect(err).NotTo(HaveOccurred())
	...
	It("allows an authorized tool call", func(ctx SpecContext) {
		...
		Expect(content).To(ContainElement(HaveKeyWithValue("text", "Hi e2e")))
	})
	...
	It("rejects an unauthorized tool call", func(ctx SpecContext) {
		...
		status, body, _, err := mcpAuthRawPost(ctx, mcpAuthGatewayURL, sessionID,
...
func mcpAuthHTTPRoute(name, hostname, serviceName string) *gatewayapiv1.HTTPRoute {
	return tests.BuildBasicHttpRoute(name, "mcp-gateway", mcpAuthTestServerNS, []string{hostname}, func(route *gatewayapiv1.HTTPRoute) {
		route.Spec.ParentRefs[0].Namespace = ptr.To(gatewayapiv1.Namespace("gateway-system"))
		route.Spec.ParentRefs[0].SectionName = ptr.To(gatewayapiv1.SectionName("mcp-tls"))
...
func mcpAuthCallTool(ctx context.Context, endpoint, sessionID, toolName string, args map[string]any, headers map[string]string) (int, []map[string]any, error) {
	...
	if callResult.IsError {
		return response.StatusCode, nil, fmt.Errorf("tools/call %q reported a tool execution error", toolName)
	}
