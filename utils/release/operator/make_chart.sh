...
wasm_shim_version=$(yq '.dependencies.Wasm_shim' $env/release.yaml)
...
consoleplugin_version=$(yq '.dependencies.Console_plugin' $env/release.yaml)
...
operator_version=$(yq '.kuadrant.release' $env/release.yaml)
V="$(yq '.kuadrant.release' $env/release.yaml)" yq --inplace eval '.version = strenv(V)' $env/charts/kuadrant-operator/Chart.yaml
V="$(yq '.kuadrant.release' $env/release.yaml)" yq --inplace eval '.appVersion = strenv(V)' $env/charts/kuadrant-operator/Chart.yaml
V="$(yq '.dependencies.Authorino' $env/release.yaml)" yq --inplace eval '(.dependencies[] | select(.name == "authorino-operator").version) = strenv(V)' $env/charts/kuadrant-operator/Chart.yaml
V="$(yq '.dependencies.Limitador' $env/release.yaml)" yq --inplace eval '(.dependencies[] | select(.name == "limitador-operator").version) = strenv(V)' $env/charts/kuadrant-operator/Chart.yaml
V="$(yq '.dependencies.DNS' $env/release.yaml)" yq --inplace eval '(.dependencies[] | select(.name == "dns-operator").version) = strenv(V)' $env/charts/kuadrant-operator/Chart.yaml
